...
const ExpandSidebarButton = ({ toggleSidebar, isCollapsed }: ExpandSidebarButtonProps) => (
    <SidebarMenuItem>
        <SidebarMenuButton asChild size="nav" className="text-slate-600 transition-colors">
            <button type="button" onClick={(e) => { e.stopPropagation(); toggleSidebar(); }} className="flex w-full items-center group-data-[state=collapsed]:justify-center group-data-[state=expanded]:mx-4 group-data-[state=expanded]:px-4 group-data-[state=expanded]:gap-4">
                <div className={cn("h-4 flex items-center justify-start shrink-0 group-data-[state=expanded]:")}>
